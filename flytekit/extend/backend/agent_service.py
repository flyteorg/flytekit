...
request_count = Counter("flyte_agent_req_count", "Total number of requests", ["task_type"])
request_success_count = Counter("flyte_agent_req_success_count", "Total number of successful requests", ["task_type"])
request_failure_count = Counter("flyte_agent_req_failure_count", "Total number of failed requests", ["task_type"])
...
class AsyncAgentService(AsyncAgentServiceServicer):
    async def CreateTask(self, request: CreateTaskRequest, context: grpc.ServicerContext) -> CreateTaskResponse:
        try:
            with request_latency.labels(task_type=request.template.type, operation="create").time():
