...
class _SdkHiveTask(_sdk_runnable.SdkRunnableTask):
    ...
    def _handle_user_returns(self, context, user_returned):
        queries_from_task = user_returned or []
        ...
        nodes = []
        tasks = []
        for i, hive_job in enumerate(generated_queries):
            hive_job_node = _create_hive_job_node(
                "HiveQuery_{}".format(i),
                hive_job.to_flyte_idl(),
                self.metadata
            )
    ...
    def _pack_output_references(self, context, outputs):
        futures = context.output_protos.get(_constants.FUTURES_FILE_NAME)
        if futures is None:
            # If no queries were produced, then the spec should not have any nodes, in which case
            # we just produce an
            # outputs file like any other single-step tasks.
            super(_SdkHiveTask, self)._pack_output_references(context, outputs)
        else:
            # If a futures file is present, instead of writing an output file, we should write to the bindings in the
            # future file.  Propeller will unpack that to outputs upon completion of the queries.
            futures.outputs[:] = [
