...
class PKCEAuthenticator(Authenticator):
    ...
    def __init__(
        self, endpoint: str, cfg_store: ClientConfigStore, header_key: str = None
    ):
        """
    ...
    def refresh_credentials(self):
        ...
        if not self._cmd:
            raise AuthenticationError(
                "No command specified in configuration for command authentication"
            )
        logging.debug(
        ...
        try:
            output = subprocess.run(
                self._cmd, capture_output=True, text=True, check=True
            )
...
class ClientCredentialsAuthenticator(Authenticator):
    ...
    @staticmethod
    def get_token(
        token_endpoint: str, authorization_header: str, scopes: typing.List[str]
    ...
    def get_basic_authorization_header(client_id: str, client_secret: str) -> str:
        ...
        return "Basic {}".format(
            base64.b64encode(
                concated.encode(ClientCredentialsAuthenticator._utf_8)
            ).decode(ClientCredentialsAuthenticator._utf_8)
        )
    ...
    def refresh_credentials(self):
        ...
        logging.debug(
            f"Basic authorization flow with client id {self._client_id} scope {scopes}"
        )
        authorization_header = self.get_basic_authorization_header(
