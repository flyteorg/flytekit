...
class PandasPersistenceHandlers(DatasetPersistenceHandler):
    def to_parquet(
        self,
        df: pandas.DataFrame,
        to_file: os.PathLike,
    ...
    def to_bigquery(
        self, df: pandas.DataFrame, path: str, job_config: typing.Optional[LoadJobConfig] = None
    ) -> LoadJob:
...
class PandasRetrievalHandler(DatasetRetrievalHandler):
    def read_parquet(self, path) -> pd.DataFrame:
        protocol = split_protocol(path)[0] or "file"
        fs = fsspec.filesystem(protocol)
        with fs.open(path) as f:
            return pandas.read_parquet(f)

    def read_bigquery(self, path: str, **kwargs) -> pd.DataFrame:
...
FLYTE_DATASET_TRANSFORMER.register_handler(pd.DataFrame, PandasPersistenceHandlers())
FLYTE_DATASET_TRANSFORMER.register_handler(pd.DataFrame, PandasRetrievalHandler())
