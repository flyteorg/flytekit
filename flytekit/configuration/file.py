...
def get_config_file(c: typing.Union[str, ConfigFile, None]) -> typing.Optional[ConfigFile]:
    ...
    if c is None:
        ...
        if current_location_config.exists():
            logger.debug(f"Using configuration from Python process root {current_location_config.absolute()}")
            return ConfigFile(current_location_config.absolute())

        # If not, see if there's a config in the user's home directory
        home = Path.home()
        home_dir_config = Path(home, ".flyte/config")  # _default_config_file_name in main.py
