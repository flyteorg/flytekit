...
def download_distribution(additional_distribution: str, destination: str):
    ...
    FlyteContextManager.current_context().file_access.get_data(additional_distribution, os.path.join("/tmp", ""))
    ...
    result = _subprocess.run(
        ["tar", "-xvf", os.path.join("/tmp", tarfile_name), Path(tarfile_name).with_suffix('').stem],
