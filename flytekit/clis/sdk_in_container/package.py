...
from flytekit.clis.sdk_in_container import serialize, constants
...
@click.option(
    "-i", "--image", "image_config", required=False, multiple=True,
...
@click.option(
    ...
    "--output",
    required=False,
    type=click.Path(exists=False, dir_okay=False, writable=True, resolve_path=True, allow_dash=True),
...
@click.pass_context
def package(ctx, image_config, output, fast, python_interpreter):
    """
