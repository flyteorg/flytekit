...
class ExecutableTemplateShimTask(object):
    ...
    * An executor, which can use information from the task template (including the ``custom`` field)
    ...
    def __init__(self, tt: _task_models.TaskTemplate, executor_type: Type[ShimTaskExecutor], *args, **kwargs):
    ...
    @property
    def task_template(self) -> _task_models.TaskTemplate:
...
class ShimTaskExecutor(TrackedInstance, Generic[T]):
    def execute_from_model(self, tt: _task_models.TaskTemplate, **kwargs) -> Any:
        """
