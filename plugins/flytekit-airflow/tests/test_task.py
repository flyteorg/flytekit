...
from flytekitplugins.airflow.task import _is_deferrable, AirflowTask, AirflowObj, AirflowContainerTask, \
...
def test_xcom_push():
    ...
    execution_state = ctx.execution_state.with_params(
        user_space_params=ctx.user_space_params.new_builder().add_attr("GET_ORIGINAL_TASK", True).add_attr("XCOM_DATA",
                                                                                                           {}).build())
...
def test_airflow_task():
    cfg = AirflowObj(module="airflow.operators.bash", name="BashOperator", parameters={"task_id": "id", "bash_command": "echo 'hello world'"})
...
def test_airflow_container_task():
    cfg = AirflowObj(module="airflow.providers.apache.beam.operators.beam",
    ...
    assert isinstance(airflow_task_resolver.load_task(t.task_resolver.loader_args(serialization_settings, t)), AirflowContainerTask)
