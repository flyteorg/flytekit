...
class ModinPandasDataFrameTransformer(TypeTransformer[pandas.DataFrame]):
    ...
    def to_literal(
        ...
        w = ModinPandasSchemaWriter(local_dir=local_dir, cols=None, fmt=SchemaFormat.PARQUET)
        w.write(python_val)
    ...
    def to_python_value(
        ...
        r = ModinPandasSchemaReader(local_dir=local_dir, cols=None, fmt=SchemaFormat.PARQUET)
        return r.all()


SchemaEngine.register_handler(
    SchemaHandler("ModinPandas.Dataframe-Schema", pandas.DataFrame, ModinPandasSchemaReader, ModinPandasSchemaWriter)
)
