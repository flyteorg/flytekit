...
from flytekitplugins.skypilot import SkyPilotAgent, SkyPilot
from flytekit.core.data_persistence import FileAccessProvider
from flytekitplugins.skypilot.agent import SkyTaskTracker, SkyTaskFuture
from flytekit.extend.backend.base_agent import AgentRegistry
from flytekit.core.data_persistence import FileAccessProvider
from flytekitplugins.skypilot.task_utils import ContainerRunType, get_sky_task_config, RunCommand, SetupCommand
...
@pytest.mark.parametrize(
    "container_run_type",
    [(ContainerRunType.APP),
    (ContainerRunType.RUNTIME)]
)
def test_skypilot_task(container_run_type):
    ...
    if container_run_type == ContainerRunType.APP:
        assert sky_task.setup == textwrap.dedent(f"""\
            {task_config.setup}
            docker pull {container.image}
            """).strip()
        assert sky_task.run.startswith(f"docker run")
...
def mock_provider(mock_fs):
    with mock.patch(
        "flytekitplugins.skypilot.agent.SkyPathSetting", autospec=True, return_value=SkyPathSetting(task_level_prefix=str(mock_fs.local_sandbox_dir), unique_id="sky_mock")
    ) as mock_path, mock.patch(
        "flytekitplugins.skypilot.agent.setup_cloud_credential", autospec=True
    ) as cloud_setup, mock.patch(
        "flytekitplugins.skypilot.agent.SkyTaskFuture.launch", autospec=True, side_effect=functools.partial(mock_launch, sleep_time=5)
    ) as mock_provider:
