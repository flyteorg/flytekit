...
def test_builtin_algorithm_training_job_task():
    ...
    assert (
        builtin_algorithm_training_job_task.interface.inputs["train"].description == ""
    )
    ...
    assert (
        builtin_algorithm_training_job_task.interface.inputs["validation"].description
        == ""
    )
    assert (
    ...
    assert (
        builtin_algorithm_training_job_task.interface.inputs[
    ...
    assert (
        builtin_algorithm_training_job_task.interface.outputs["model"].description == ""
    )
    assert (
    ...
    assert (
        builtin_algorithm_training_job_task.type
        == _common_constants.SdkTaskType.SAGEMAKER_TRAINING_JOB_TASK
    )
    ...
    assert (
        "metricDefinitions"
        not in builtin_algorithm_training_job_task.custom[
            "algorithmSpecification"
        ].keys()
    )
...
def test_simple_hpo_job_task():
    ...
    assert (
        simple_xgboost_hpo_job_task.interface.inputs[
            "static_hyperparameters"
        ].description
        == ""
    )
    ...
    assert (
        simple_xgboost_hpo_job_task.interface.outputs["model"].type
        == _sdk_types.Types.Blob.to_flyte_literal_type()
    )
    assert (
        simple_xgboost_hpo_job_task.type
        == _common_constants.SdkTaskType.SAGEMAKER_HYPERPARAMETER_TUNING_JOB_TASK
    )
    ...
    assert (
        "metricDefinitions"
        in simple_xgboost_hpo_job_task.custom["trainingJob"][
            "algorithmSpecification"
        ].keys()
    )
    assert (
        len(
            simple_xgboost_hpo_job_task.custom["trainingJob"]["algorithmSpecification"][
                "metricDefinitions"
            ]
        )
        == 1
