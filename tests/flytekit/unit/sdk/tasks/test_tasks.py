...
def test_default_python_task():
    ...
    assert default_task.interface.inputs["in1"].type == type_models.LiteralType(simple=type_models.SimpleType.INTEGER)
    assert default_task.interface.outputs["out1"].description == ""
    assert default_task.interface.outputs["out1"].type == type_models.LiteralType(simple=type_models.SimpleType.STRING)
...
def test_default_resources():
    with _configuration.TemporaryConfiguration(
        ...
        assert request_map[Resources.ResourceName.CPU] == "500m"
        assert request_map[Resources.ResourceName.MEMORY] == "500Gi"
        assert request_map[Resources.ResourceName.GPU] == "1"
        assert request_map[Resources.ResourceName.STORAGE] == "500Gi"

        assert limit_map[Resources.ResourceName.CPU] == "501m"
        assert limit_map[Resources.ResourceName.MEMORY] == "501Gi"
        assert limit_map[Resources.ResourceName.GPU] == "2"
        assert limit_map[Resources.ResourceName.STORAGE] == "501Gi"
...
def test_overriden_resources():
    with _configuration.TemporaryConfiguration(
        ...
        assert request_map[Resources.ResourceName.CPU] == "500m"
        assert request_map[Resources.ResourceName.MEMORY] == "50Gi"
        assert request_map[Resources.ResourceName.GPU] == "0"
        assert request_map[Resources.ResourceName.STORAGE] == "100Gi"

        assert limit_map[Resources.ResourceName.CPU] == "1000m"
        assert limit_map[Resources.ResourceName.MEMORY] == "100Gi"
        assert limit_map[Resources.ResourceName.GPU] == "1"
        assert limit_map[Resources.ResourceName.STORAGE] == "200Gi"
