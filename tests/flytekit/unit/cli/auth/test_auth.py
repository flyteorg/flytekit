...
def test_create_code_challenge():
    ...
    assert (
        _auth._create_code_challenge(test_code_verifier)
        == "Qq1fGD0HhxwbmeMrqaebgn1qhvKeguQPXqLdpmixaM4"
    )


def test_oauth_http_server():
    queue = _Queue()
    server = _auth.OAuthHTTPServer(
        ("localhost", 9000), _BaseHTTPServer.BaseHTTPRequestHandler, queue=queue
    )
