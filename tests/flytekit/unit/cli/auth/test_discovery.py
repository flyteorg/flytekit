...
def test_get_authorization_endpoints():
    ...
    assert (
        discovery_client.get_authorization_endpoints().token_endpoint == token_endpoint
    )
...
def test_get_authorization_endpoints_relative():
    ...
    discovery_client = _discovery.DiscoveryClient(discovery_url=discovery_url)
    assert (
        discovery_client.get_authorization_endpoints().auth_endpoint
        == "http://flyte-admin.com/authorization"
    )
    assert (
        discovery_client.get_authorization_endpoints().token_endpoint
        == "http://flyte-admin.com/token"
    )
