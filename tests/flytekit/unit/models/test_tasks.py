...
def test_resource_entry():
    obj = task_models.Resources.ResourceEntry(task_models.Resources.ResourceName.CPU, "blah")
    assert task_models.Resources.ResourceEntry.from_flyte_idl(obj.to_flyte_idl()) == obj
    assert obj != task_models.Resources.ResourceEntry(task_models.Resources.ResourceName.GPU, "blah")
    assert obj != task_models.Resources.ResourceEntry(task_models.Resources.ResourceName.CPU, "bloop")
    assert obj.name == task_models.Resources.ResourceName.CPU
...
def test_resources(resource_list):
    obj = task_models.Resources(resource_list, resource_list)
    obj1 = task_models.Resources([], resource_list)
    obj2 = task_models.Resources(resource_list, [])

    assert obj.requests == obj2.requests
    assert obj.limits == obj1.limits
    assert obj == task_models.Resources.from_flyte_idl(obj.to_flyte_idl())
...
def test_task_metadata():
    obj = _taskMetadata(
        ...
        timedelta(days=1),
        literal_models.RetryStrategy(3),
        True,
        "0.1.1b0",
        "This is deprecated!",
    )
...
def test_task_template__k8s_pod_target():
    int_type = type_models.LiteralType(type_models.SimpleType.INTEGER)
    obj = _taskTemplate(
        identifier.Identifier(identifier.ResourceType.TASK, "project", "domain", "name", "version"),
        "python",
        _taskMetadata(
        ...
        {"a": 1, "b": {"c": 2, "d": 3}},
        config={"a": "b"},
        k8s_pod=task_models.K8sPod(
            metadata=task_models.K8sObjectMetadata(labels={"label": "foo"}, annotations={"anno": "bar"}),
    ...
    assert obj.type == "python"
    assert obj.custom == {"a": 1, "b": {"c": 2, "d": 3}}
    assert obj.k8s_pod.metadata == task_models.K8sObjectMetadata(labels={"label": "foo"}, annotations={"anno": "bar"})
...
def test_container(resources):
    ...
    obj.env == {"a": "b"}
    obj.config == {"d": "e"}
    assert obj == task_models.Container.from_flyte_idl(obj.to_flyte_idl())
...
def test_sidecar_task():
    ...
    obj2 = task_models.SidecarJob.from_flyte_idl(obj.to_flyte_idl())
    assert obj2 == obj


def test_sidecar_task_label_annotation_not_provided():
    pod_spec = generated_pb2.PodSpec()
    obj = task_models.SidecarJob(pod_spec=pod_spec, primary_container_name="primary")
...
def test_dataloadingconfig():
    dlc = task_models.DataLoadingConfig(
        "s3://input/path",
        "s3://output/path",
        True,
        task_models.DataLoadingConfig.LITERALMAP_FORMAT_YAML,
    )
    dlc2 = task_models.DataLoadingConfig.from_flyte_idl(dlc.to_flyte_idl())
    assert dlc2 == dlc
    ...
    dlc = task_models.DataLoadingConfig(
        ...
        True,
        task_models.DataLoadingConfig.LITERALMAP_FORMAT_YAML,
        io_strategy=task_models.IOStrategy(),
    )
    dlc2 = task_models.DataLoadingConfig.from_flyte_idl(dlc.to_flyte_idl())
    assert dlc2 == dlc
...
def test_ioconfig():
    io = task_models.IOStrategy(
        task_models.IOStrategy.DOWNLOAD_MODE_NO_DOWNLOAD,
        task_models.IOStrategy.UPLOAD_MODE_NO_UPLOAD,
    )
    assert io == task_models.IOStrategy.from_flyte_idl(io.to_flyte_idl())


def test_k8s_metadata():
    obj = task_models.K8sObjectMetadata(labels={"label": "foo"}, annotations={"anno": "bar"})
    assert obj.labels == {"label": "foo"}
    assert obj.annotations == {"anno": "bar"}
    assert obj == task_models.K8sObjectMetadata.from_flyte_idl(obj.to_flyte_idl())


def test_k8s_pod():
    obj = task_models.K8sPod(
        metadata=task_models.K8sObjectMetadata(labels={"label": "foo"}), pod_spec={"pod_spec": "bar"}
    )
    assert obj.metadata.labels == {"label": "foo"}
    assert obj.pod_spec == {"pod_spec": "bar"}
    assert obj == task_models.K8sPod.from_flyte_idl(obj.to_flyte_idl())
