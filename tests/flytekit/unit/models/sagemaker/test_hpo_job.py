from flytekit.models.sagemaker import hpo_job
from flytekit.models.sagemaker import parameter_ranges
from flytekit.models.sagemaker import training_job


def test_hyperparameter_tuning_objective():
    obj = hpo_job.HyperparameterTuningObjective(objective_type=hpo_job.HyperparameterTuningObjectiveType.MAXIMIZE, metric_name="test_metric")
...
def test_hyperparameter_job_config():
    jc = hpo_job.HyperparameterTuningJobConfig(
        hyperparameter_ranges=parameter_ranges.ParameterRanges(
            parameter_range_map={
                "a": parameter_ranges.CategoricalParameterRange(values=["1", "2"]),
                "b": parameter_ranges.IntegerParameterRange(
                    min_value=0,
                    max_value=10,
                    scaling_type=parameter_ranges.HyperparameterScalingType.LINEAR)
            }),
...
def test_hyperparameter_tuning_job():
    ...
    assert (hpo2.training_job.training_job_resource_config.instance_type ==
            hpo.training_job.training_job_resource_config.instance_type)
    assert (hpo2.training_job.training_job_resource_config.instance_count ==
            hpo.training_job.training_job_resource_config.instance_count)
    assert (hpo2.training_job.training_job_resource_config.distributed_protocol ==
            hpo.training_job.training_job_resource_config.distributed_protocol)
    assert (hpo2.training_job.training_job_resource_config.volume_size_in_gb ==
            hpo.training_job.training_job_resource_config.volume_size_in_gb)
    assert (hpo2.training_job.algorithm_specification.algorithm_name ==
            hpo.training_job.algorithm_specification.algorithm_name)
    assert (hpo2.training_job.algorithm_specification.algorithm_version ==
            hpo.training_job.algorithm_specification.algorithm_version)
    assert (hpo2.training_job.algorithm_specification.input_mode ==
            hpo.training_job.algorithm_specification.input_mode)
    assert (hpo2.training_job.algorithm_specification.input_content_type ==
