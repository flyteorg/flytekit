...
def test_flyte_not_implemented_exception():
    try:
        ...
        assert str(e) == "SYSTEM:NotImplemented: error=I'm lazy so I didn't implement this., cause=somewhere else didn't implement this"
...
def test_flyte_entrypoint_not_loadable_exception():
    ...
    try:
        base_exn = Exception("somewhere else couldn't load this")
        raise system.FlyteEntrypointNotLoadable("fake.module", additional_msg="Shouldn't have used a fake module!") from base_exn
    except Exception as e:
        assert (
            str(e) == "SYSTEM:UnloadableCode: error=Entrypoint is not loadable!  Could not load the module: 'fake.module' "
    ...
    try:
        ...
        assert (
            str(e) == "SYSTEM:UnloadableCode: error=Entrypoint is not loadable!  Could not find the task: 'secret_task' in 'fake.module' "
...
def test_flyte_system_assertion():
    try:
        ...
        assert str(e) == "SYSTEM:AssertionError: error=I assert that the system messed up., cause=somewhere else asserts this is wrong"
