...
def test_flyte_type_exception():
    try:
        ...
        raise user.FlyteTypeException("int", "float", received_value=1, additional_msg="That was a bad idea!") from base_exn
    except Exception as e:
        assert (
    ...
    try:
        ...
        assert (
            str(e) == "USER:TypeError: error=Type error!  Received: int with value: 1, Expected one of: ('list', 'set'). That was a "
    ...
    try:
        ...
        assert str(e) == "USER:TypeError: error=Type error!  Received: int, Expected: float. That was a bad idea!, cause=int != float"
        assert isinstance(e, TypeError)
        assert type(e).error_code == "USER:TypeError"
        assert isinstance(e, user.FlyteUserException)
    ...
    try:
        ...
        assert str(e) == "USER:TypeError: error=Type error!  Received: int, Expected one of: ('list', 'set'). That was a " "bad idea!"
        assert isinstance(e, TypeError)
        assert type(e).error_code == "USER:TypeError"
        assert isinstance(e, user.FlyteUserException)
...
def test_flyte_value_exception():
    try:
        ...
        assert str(e) == "USER:ValueError: error=Value error!  Received: -1. Expected a value > 0, cause=live up to expectations"
...
def test_flyte_validation_error():
    try:
        ...
        assert str(e) == "USER:ValidationError: error=I validated that your stuff was wrong., cause=somewhere else said this isn't valid"
        assert isinstance(e, AssertionError)
