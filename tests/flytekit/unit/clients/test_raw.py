...
def test_refresh_credentials_basic(mock_credentials_access, mock_requests):
    mock_credentials_access.get_authorization_endpoints.return_value = _AuthorizationEndpoints(
        "auth", "token"
    )
    ...
    mock_credentials_access.get_authorization_endpoints.assert_called_with(
        mock_client.url
    )
