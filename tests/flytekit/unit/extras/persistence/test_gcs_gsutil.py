...
def test_put(mock_check, mock_exec):
    ...
    mock_exec.assert_called_with(["gsutil", "cp", "/test", "gs://my-bucket/k1"])
...
def test_put_recursive(mock_check, mock_exec):
    ...
    mock_exec.assert_called_with(["gsutil", "cp", "-r", "/test/*", "gs://my-bucket/k1/"])
...
def test_get(mock_check, mock_exec):
    ...
    mock_exec.assert_called_with(["gsutil", "cp", "gs://my-bucket/k1", "/test"])
...
def test_get_recursive(mock_check, mock_exec):
    ...
    mock_exec.assert_called_with(["gsutil", "cp", "-r", "gs://my-bucket/k1/*", "/test"])
