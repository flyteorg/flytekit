...
def test_all_types_in_pydantic_basemodel_wf(local_dummy_file, local_dummy_directory):
    class InnerBM(BaseModel):
        ...
        c: str = "Hello, Flyte"
        d: bool = False
        e: dict = Field(default_factory=lambda: {"key": "value"})
        f: FlyteFile = Field(default_factory=lambda: FlyteFile(local_dummy_file))
        g: FlyteDirectory = Field(
            default_factory=lambda: FlyteDirectory(local_dummy_directory)
        )
    ...
    def t_inner(inner_bm: InnerBM):
        ...
        assert type(inner_bm.f) is FlyteFile
        with open(inner_bm.f, "r") as f:
            assert f.read() == "Hello FlyteFile"

        assert type(inner_bm.g) is FlyteDirectory
        assert not inner_bm.g.downloaded
        with open(os.path.join(inner_bm.g, "file"), "r") as fh:
            assert fh.read() == "Hello FlyteDirectory"
        assert inner_bm.g.downloaded
    ...
    def t_test_all_attributes(
        ...
        assert isinstance(e, dict), "m is not dict"

        # Strict type check for FlyteFile
        assert isinstance(f, FlyteFile), "n is not FlyteFile"

        # Strict type check for FlyteDirectory
        assert isinstance(g, FlyteDirectory), "o is not FlyteDirectory"
