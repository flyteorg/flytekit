...
def test_notebook_task_simple():
    ...
    nb = NotebookTask(
        name="test",
        notebook_path=f"./testdata/{nb_name}.ipynb",
        inputs=kwtypes(pi=float),
        outputs=kwtypes(square=float),
    )
    sqr, out, render = nb.execute(pi=4)
    out_path = os.path.abspath(f"./testdata/{nb_name}-out.ipynb")
    render_path = os.path.abspath(f"./testdata/{nb_name}-out.html")
    assert sqr == 16.0
...
def test_notebook_task_multi_values():
    ...
    assert nb.output_notebook_path == out == os.path.abspath(f"./testdata/{nb_name}-out.ipynb")
    assert nb.rendered_output_path == render == os.path.abspath(f"./testdata/{nb_name}-out.html")
...
def test_notebook_task_complex():
    ...
    h, w, x, out, render = nb.execute(h="blah", n=10, w=os.path.abspath("./testdata/nb-multi.ipynb"))
    assert h == "blah world!"
